# Stage 2: backfill full content (transcripts / article text) for DB rows that don't have it yet
...
if __name__ == "__main__":
    filled = run()
    for source, count in filled.items():
        print(f"{source}: {count} article(s) backfilled")
